...
- **Discovers courses from repo names alone** — an assignment is a shared repo-name
  prefix; there is no state to set up until you want some
...
- **Managed classrooms** — an optional
  [classroom-meta repo](docs/commands.md#the-classroom-meta-repo) records teams per
  assignment, creates private repos (optionally from a template), keeps student, TA, and
  branch-protection state reconciled, and tracks repos by permanent id so renames can't
  hide them
