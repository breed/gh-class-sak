...
- each org may have a private repo named `classroom-meta` recording classroom state: one directory per classroom with `classroom.ini` ([CLASSROOM] optional prefix, optional template, optional repo settings), `tas` (one identity per line), and one `<assignment>.tsv` per assignment (NAME STUDENTS REPO REPO_ID)
